...
# -*- coding: utf-8 -*-
r"""
run_heldout.py  --  Turn the grokking DELAY LAW from a single-p curve into a SCALING LAW.
...
USAGE (PowerShell):
  python run_heldout.py --dry-run
  python run_heldout.py --quick --device cuda --out_root D:\Colab-local
  python run_heldout.py --device cuda --out_root D:\Colab-local
  python run_heldout.py --resume D:\Colab-local\paperA_heldout_2026...
  python analyze_heldout.py --root D:\Colab-local\paperA_heldout_2026...
"""
...
def budget_for(p, rho):
    # generous (>=3x the higher-model prediction) so held-out cells finish uncensored regardless of which
    # alpha-model holds; this is sizing only, not a scientific choice.
    sched = {0.85: 8_000, 1.00: 15_000, 1.15: 40_000}
    return sched.get(round(rho, 2), 15_000)

def default_grid():
    # HELD-OUT prediction test: p=67 (interpolation, inside fitted 41-71) and p=97 (extrapolation, outside).
    # Predictions are PREREGISTERED in heldout_prediction_*.json (predict_heldout.py) before this is run.
    d, H = 128, 256
    ...
    for p in (67, 97):
        ...
        for rho in (0.85, 1.00, 1.15):                    # exponential-regime rho only (law's stated range)
...
def main():
    ...
    root = args.resume or os.path.join(args.out_root,
            f"paperA_heldout_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}")
