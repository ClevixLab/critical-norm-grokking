...
# -*- coding: utf-8 -*-
r"""
run_denserho.py  --  Turn the grokking DELAY LAW from a single-p curve into a SCALING LAW.
...
USAGE (PowerShell):
  python run_denserho.py --dry-run
  python run_denserho.py --quick --device cuda --out_root D:\Colab-local
  python run_denserho.py --device cuda --out_root D:\Colab-local
  python run_denserho.py --resume D:\Colab-local\paperA_denserho_2026...
  python analyze_denserho.py --root D:\Colab-local\paperA_denserho_2026...
"""
...
def budget_for(p, rho):
    # Calibrated to the MEASURED p=59 timings (T_grok ~ 1439/4053/13323/28205 at rho=0.85/1.0/1.15/1.30),
    # log-interpolated across the dense rho grid and given ~3x headroom (floor 8k), so every cell finishes
    # uncensored without wasting compute. config_id excludes max_steps -> resume-safe.
    sched = {0.85: 8_000, 0.90: 8_000, 0.95: 9_000, 1.00: 12_000, 1.05: 18_000,
             1.10: 27_000, 1.15: 40_000, 1.20: 51_000, 1.25: 66_000, 1.30: 60_000}
    return sched.get(round(rho, 2), 40_000)

def default_grid():
    # DENSE rho sweep at a SINGLE task size (p=59) to (i) move the exponential fit from 3 to 10 distinct
    # rho values, (ii) locate the exponential->saturation knee precisely (currently only known to lie between
    # 1.15 and 1.30), (iii) give tight bootstrap CIs on alpha. 16 seeds for CI quality.
    d, H, p = 128, 256, 59
    seeds, t_int, lr, lam, alpha = 16, 500, 1e-3, 1.0, 0.40
    rhos = [0.85, 0.90, 0.95, 1.00, 1.05, 1.10, 1.15, 1.20, 1.25, 1.30]
    def mk(arm, rho):
        ms = budget_for(p, 1.00 if arm == "none" else round(rho, 2))
    ...
    grid = [mk("none", 1.00)]                       # control FIRST -> measures ||W||_c(p=59)
    grid += [mk("clamp", r) for r in rhos]          # 10 dense clamp cells
    return dedupe(grid)
...
def quick_grid():
    ...
    return dedupe([mk(23, "none", 1.00, 4000), mk(23, "clamp", 0.90, 4000), mk(23, "clamp", 1.10, 6000)])
...
def main():
    ...
    root = args.resume or os.path.join(args.out_root,
            f"paperA_denserho_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}")
    ...
    for i, c in enumerate(grid):
        ...
        try:
            ...
            if device != "cpu": torch.cuda.empty_cache(); torch.cuda.synchronize()
        except Exception:
            pass
