...
def default_grid():
    d, nh, H = 128, 4, 512
    base = dict(optim="adamw", p=59, alpha=0.40, d_model=d, n_heads=nh, H=H,
                lam=1.0, lr=5e-4, seeds=16, t_int=600, max_steps=30_000)
    grid = [dict(base, arm="none", rho=1.0)]                 # control FIRST (measures ||W||_c)
    for rho in [0.85, 1.00, 1.15, 1.30, 1.50]:
        grid.append(dict(base, arm="clamp", rho=rho))
    return dedupe(grid)
...
def main():
    ...
    root = args.resume or os.path.join(args.out_root,
            f"paperA_tfnoln_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}")
